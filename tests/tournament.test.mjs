...
test('La plantilla genérica crea un cuadro de 16 lugares y 15 turnos',()=>{const s=seedState();assert.equal(s.matches.length,15);assert.equal(s.matches[0].scheduledAt,'2026-10-01T08:00');assert.equal(s.matches[7].scheduledAt,'2026-10-01T12:40');assert.equal(s.matches[8].scheduledAt,'2026-10-01T13:20');assert.equal(s.matches[14].scheduledAt,'2026-10-01T17:20');assert.equal(s.config.sport,'Deporte');assert.equal(s.config.scoringMode,'winner');assert.equal(s.config.logoUrl,'');});
test('Todos los tamaños de cuadro llegan a un único campeón',()=>{for(const size of [2,4,8,16,32]){let s=loaded();s.teams=Array.from({length:size},(_,i)=>({id:i+1,name:'Pareja '+(i+1)}));s.matches=makeMatches(size);assert.equal(s.matches.length,size-1);for(const m of s.matches){const ids=sides(s,m);assert.ok(ids.every(Boolean));s=action(s,'result',{id:m.id,winnerId:ids[0],score:'6-4',reason:'played'});}assert.equal(s.matches.at(-1).winnerId,1);assert.ok(s.matches.every(m=>m.status==='finished'));assert.equal(roundName(s.matches.at(-1).round,size),'Final');}});
test('No inicia partidos con nombres o clasificados pendientes',()=>{const s=seedState();assert.throws(()=>action(s,'status',{id:3,status:'live'}),/completá/);assert.throws(()=>action(s,'status',{id:9,status:'live'}),/completá/);});
test('Una cancha solo puede tener un partido llamado o en juego',()=>{let s=loaded();s=action(s,'status',{id:1,status:'called'});assert.throws(()=>action(s,'status',{id:2,status:'live'}),/cancha/);s=action(s,'status',{id:1,status:'live'});assert.equal(s.matches[0].startedAt,'2026-09-05T12:00:00.000Z');s=action(s,'status',{id:1,status:'scheduled'});assert.equal(s.matches[0].startedAt,null);s=action(s,'status',{id:2,status:'live'});assert.equal(s.matches[1].status,'live');});
test('Ganadores avanzan y resultados no invalidan rondas que ya comenzaron',()=>{let s=loaded();s=action(s,'result',{id:1,winnerId:1,score:'6-0',reason:'played'});s=action(s,'result',{id:2,winnerId:4,score:'6-2',reason:'played'});assert.deepEqual(sides(s,s.matches[8]),[1,4]);s=action(s,'status',{id:9,status:'live'});assert.throws(()=>action(s,'reopen',{id:1,confirm:true}),/posteriores/);assert.throws(()=>action(s,'result',{id:1,winnerId:2,score:'6-0',reason:'played'}),/posterior/);s=action(s,'status',{id:9,status:'scheduled'});s=action(s,'reopen',{id:1,confirm:true});assert.deepEqual(sides(s,s.matches[8]),[null,4]);});
test('La ausencia exige motivo; nunca se declara automáticamente por horario',()=>{let s=loaded();assert.throws(()=>action(s,'result',{id:1,winnerId:1,reason:'walkover'}),/motivo/);s=action(s,'result',{id:1,winnerId:1,reason:'walkover',note:'Ausencia tras 5 minutos; no hay partido disponible para adelantar.'});assert.equal(s.matches[0].reason,'walkover');assert.equal(s.matches[1].status,'scheduled');});
test('Pase libre explícito solo con una vacante y un participante',()=>{let s=seedState();s.teams[4].name='Equipo 5';s=action(s,'bye',{id:3,winnerId:5});assert.equal(s.matches[2].winnerId,5);assert.throws(()=>action(seedState(),'bye',{id:1,winnerId:1}),/exactamente/);const both=seedState();both.teams[6].name='Equipo 7';both.teams[7].name='Equipo 8';assert.throws(()=>action(both,'bye',{id:4,winnerId:7}),/exactamente/);});
test('Cambiar tamaño no pierde participantes ni altera un cuadro iniciado',()=>{const named=seedState();named.teams[15].name='Fuera del nuevo cuadro';assert.throws(()=>action(named,'resize',{size:8,confirm:true}),/fuera/);let s=loaded();s=action(s,'status',{id:1,status:'called'});assert.throws(()=>action(s,'resize',{size:32,confirm:true}),/comenzó/);});
test('Múltiples canchas y reprogramación respetan dependencias y resultados',()=>{let s=loaded();s=action(s,'config',{courts:2});s=action(s,'replan',{start:'2026-09-05T08:00',confirm:true});assert.equal(s.matches[0].scheduledAt,s.matches[1].scheduledAt);assert.notEqual(s.matches[0].court,s.matches[1].court);for(const m of s.matches.filter(m=>m.round>1)){for(const source of [m.sourceA,m.sourceB]){const parent=s.matches.find(x=>x.id===source);assert.ok(Date.parse(m.scheduledAt+'Z')>=Date.parse(parent.scheduledAt+'Z')+parent.duration*60000);}}s=action(s,'result',{id:1,winnerId:1,reason:'played',score:'6-3'});const finished=structuredClone(s.matches[0]);s=action(s,'replan',{start:'2026-09-05T10:00',confirm:true});assert.deepEqual(s.matches[0],finished);});
test('Reprogramar no cambia cruces, ganador ni IDs',()=>{const original=loaded();const s=action(original,'schedule',{id:2,scheduledAt:'2026-10-01T07:30',court:1,duration:35,note:'Adelanto autorizado'});assert.deepEqual(sides(s,s.matches[1]),[3,4]);assert.equal(original.matches[1].scheduledAt,'2026-10-01T08:40');assert.equal(s.matches[1].scheduledAt,'2026-10-01T07:30');});
test('Valida fechas reales, precios y enlaces y rechaza acciones inesperadas',()=>{const s=seedState();assert.throws(()=>action(s,'config',{logoUrl:'http://example.com/logo.png'}),/logotipo/);assert.throws(()=>action(s,'config',{mapsUrl:'javascript:alert(1)'}),/HTTPS/);assert.throws(()=>action(s,'config',{date:'2026-02-30'}),/inválida/);assert.throws(()=>action(s,'config',{whatsappUrl:'https://evil.test'}),/WhatsApp/);assert.throws(()=>action(s,'menu',{items:[{name:'Agua',price:-1,available:true}]}),/rango/);assert.throws(()=>action(s,'unknown',{}),/permitida/);});
test('Guardar no muta la versión anterior y aumenta la revisión',()=>{const s=seedState();const n=action(s,'config',{announcement:'Demora de 15 minutos'});assert.equal(s.config.announcement,'');assert.equal(n.config.announcement,'Demora de 15 minutos');assert.equal(n.revision,1);});


test('No se puede mover un llamado a una cancha que ya está ocupada',()=>{let s=loaded();s=action(s,'config',{courts:2});s=action(s,'schedule',{id:2,scheduledAt:'2026-09-05T08:00',court:2,duration:40});s=action(s,'status',{id:1,status:'live'});s=action(s,'status',{id:2,status:'called'});assert.throws(()=>action(s,'schedule',{id:2,scheduledAt:'2026-09-05T08:00',court:1,duration:40}),/cancha/);});



test('Reprograma a jornadas no consecutivas sin exceder el horario de cierre',()=>{
...
test('Falta de capacidad no guarda un cronograma parcial',()=>{
...
test('Valida fechas duplicadas, intervalos, cantidad y eliminación de una jornada activa',()=>{
 const s=loaded();
...
test('Dos canchas respetan las jornadas y las dependencias incluso al saltar de día',()=>{
...
test('Calcula automáticamente la ganadora por sets y valida el formato',()=>{
...
test('No inventa el número de sets y las ausencias siguen disponibles',()=>{
...
test('Migra datos existentes sin perder nombres, horarios ni resultados y es idempotente',()=>{
 const old=loaded();old.schemaVersion=1;delete old.config.days;delete old.config.setsToWin;
 ...
 assert.deepEqual(old,snapshot);assert.equal(migrated.schemaVersion,4);assert.equal(migrated.revision,7);assert.equal(migrated.config.logoUrl,'');
...
test('El modo genérico registra ganador y marcador sin exigir sets',()=>{
