...
export function seedState() {
 ...
 return {schemaVersion:3,revision:0,updatedAt:null,config:{title:'Mi torneo',brand:'GESTOR DE TORNEOS',subtitle:'Programación, resultados y toda la información del evento.',sport:'Deporte',participantSingular:'Equipo',participantPlural:'Equipos',playingAreaSingular:'Cancha',playingAreaPlural:'Canchas',currency:'PYG',scoringMode:'winner',posterUrl:'',tagline:'UN TORNEO. UNA EXPERIENCIA COMPARTIDA.',date:'2026-10-01',startTime:'08:00',venue:'Sede por confirmar',address:'Dirección por confirmar',mapsUrl:'',whatsappUrl:'',contact:'',courts:1,days:[{date:'2026-10-01',startTime:'08:00',endTime:'18:00'}],setsToWin:null,duration:40,entryFee:0,prize:0,prizeExtra:'',announcement:'',rules:DEFAULT_RULES.join('\n\n'),scoring:'La organización confirmará el sistema de puntuación.',foodNote:'La oferta de comidas y bebidas se publicará aquí.'},teams:names.map((name,i)=>({id:i+1,name})),matches:makeMatches(16,'2026-10-01T08:00'),menu:[]};
}
export function roundName(round,size) { const remaining=size/2**(round-1); return ({2:'Final',4:'Semifinales',8:'Cuartos de final',16:'Octavos de final',32:'Dieciseisavos'})[remaining]||'Ronda '+round; }
...
export function teamLabel(state,id,fallback) {return state.teams.find(t=>t.id===id)?.name||(fallback!==undefined?fallback:(state.config.participantSingular||'Equipo')+' por confirmar');}
export function participantLabel(state,match,side) { const id=sides(state,match)[side]; const source=side===0?match.sourceA:match.sourceB;return id?teamLabel(state,id):source?'Ganador del partido '+String(source).padStart(2,'0'):(state.config.participantSingular||'Equipo')+' por confirmar'; }
...
export function scoringLabel(config){if(config.scoringMode==='winner')return config.scoring||'La organización confirma al ganador y el marcador.';if(config.setsToWin===2&&/super[ -]tie-break/i.test(config.scoring))return '2 sets + Super tie-break · a 7 puntos si empatan 1–1';return config.setsToWin?'Gana quien obtiene '+config.setsToWin+' '+(config.setsToWin===1?'set':'sets')+' · '+(config.setsToWin===1?'a un set':'al mejor de '+(config.setsToWin*2-1)):'Por sets · cantidad para ganar pendiente de confirmación';}
export function migrateState(original){
 ...
 s.config.rules=s.config.rules.replace('Formato de puntuación (sets, games y desempate): pendiente de confirmación por la organización.',DEFAULT_RULES.at(-1));
...
export function applyAction(original,action,now=new Date().toISOString()) {
 ...
 switch(action.type){
  case 'config': {
   ...
   if(cfg.posterUrl){check(cfg.posterUrl.startsWith('/')||/^https:\/\/[^\s]+$/i.test(cfg.posterUrl),'El afiche debe ser una ruta local o un enlace HTTPS.');}
